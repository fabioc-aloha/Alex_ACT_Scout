...
name: plan
description: >-
  Use when the user wants a plan instead of execution, or before any non-trivial implementation
  (multi-file, architectural choice, > 15 min). Writes a concrete actionable markdown plan with
  bite-sized tasks (2-5 min each), exact file paths, complete code, and verification steps.
  Adapted from Hermes Agent / obra/superpowers.
---
...
If the active runtime forbids creating planning Markdown in the repository, use the session plan file or chat response instead. Only create a repository Markdown plan when the user explicitly asks for that artifact or the current project convention allows it.
