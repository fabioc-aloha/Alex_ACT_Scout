...
description: >-
  Structured skepticism for high-stakes decisions and reviews. Six methods — Red Team/Blue Team,
  Pre-Mortem, Steel Manning, Murphyjitsu, 10/10/10, Cross-Model External Critic. Use before
  publishing an ADR, plan, proposal, framework edit, or release; when "this seems too good to be
  true" fires; when groupthink is forming; or when routing critique to a different model or
  context to escape shared-context blind spots.
---
...
**Important — this brain is not code-focused.** Built-in review helpers are often engineered primarily for **code** artifacts. This brain fires on decisions, prose, plans, ADRs, framework edits, book chapters, curation moves — code is a downstream consequence of project needs, not the primary target. Route the critique to the mechanism that matches the artifact.

**Environment × artifact routing**:

| Artifact type                                  | Scout / CLI route                                                                                      | Manual fallback                                                                             |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Code (function, PR, refactor, security review) | Use a code-review or rubber-duck subagent/model when available; provide the diff and ask for critique.  | Start a fresh chat/model with the diff and "critique this change, don't defend it."          |
| Prose / decision / plan / ADR / framework edit | Use a general-purpose or critique-capable subagent/model with fresh context when available.             | Start a fresh chat/model with the artifact and "argue the strongest case against this."      |
| Book chapter / creative / long-form            | Use a general-purpose critic with explicit "skeptical reader" framing.                                 | Start a fresh chat/model with the same framing.                                             |
| Architectural direction / trade-off            | Compare two independent critiques when the tool/runtime supports it.                                   | Run two separate fresh-context critiques and compare.                                       |
