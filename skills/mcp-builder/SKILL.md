...
name: mcp-builder
description: >-
  Build MCP servers for LLM tool integration in Python (FastMCP), Node/TypeScript (MCP SDK), or
  C#/.NET (Microsoft MCP SDK). Use when authoring a new MCP server, evaluating a candidate MCP for
  the Plugin Mall, or diagnosing an MCP issue raised by a heir.
---
