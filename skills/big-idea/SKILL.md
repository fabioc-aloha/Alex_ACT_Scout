...
name: big-idea
description: >-
  Distill the central claim before authoring any summary-shaped output: hero copy, commit-message
  subject, PR title, ADR title, slide title, executive summary, skill description, abstract.
  Length is format-dependent (1 sentence for titles, up to a short paragraph for complex theses),
  tested against the Saint-Exupéry rule (remove sentences until removing another would break the
  claim). Chart summaries use the specialized chart-big-idea skill in the flint-chart-plugin.
---
...
Generalized from `chart-big-idea`, which specializes this discipline for chart authoring. This skill covers everything else.
...
- `chart-big-idea` — chart specialization. Both skills share the same six-step arc with offset numbering: chart-big-idea Step 0 (read context) → this skill's Step 1; chart-big-idea Steps 1-4 → this skill's Steps 2-5; chart-big-idea Step 5 emits a Chart Brief while this skill's Step 6 emits prose.
