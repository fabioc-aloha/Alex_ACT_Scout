...
name: spike
description: >-
  Use when the user wants to feel out an idea before committing to a real build — "spike this
  out", "try it", "is this even possible?", "compare A vs B". Throwaway experiments that decompose
  into 2-5 feasibility questions, build minimal observable prototypes, and return VALIDATED /
  PARTIAL / INVALIDATED verdicts. Disposable by design. Adapted from Hermes Agent / GSD.
---
...
In Scout runtimes that forbid creating planning/prototype Markdown in the repository unless explicitly requested, build disposable spike artifacts in a temporary/session location and summarize results in chat. Only create `spikes/` or `README.md` files inside the repository when the user explicitly asks for those artifacts or the project convention allows them.
...
**Parallel comparison spikes (002a / 002b) — delegate when available.** When two approaches can run in parallel and both need real engineering (not 10-line prototypes), fan out to worker subagents if the runtime supports them. Each subagent returns its own verdict; you write the head-to-head.

### 5. Verdict

Each spike summary closes with:
...
- **Date-based**: 2026-11-30 (v2.0.1 tester-feedback window). If by then `spike` is invoked but testers report the decomposition step (2-5 feasibility questions) consistently feels too heavy for their work shape, simplify the table-driven approach.
