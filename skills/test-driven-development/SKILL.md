...
description: >-
  Use for any feature, bug fix, refactor, or behavior change — enforces RED-GREEN-REFACTOR. Write
  the failing test first, watch it fail, write minimal code to pass, refactor. Carve out only
  throwaway prototypes, generated code, configuration files. Adapted from Hermes Agent /
  obra/superpowers.
---
...
- Delegated work — enforce TDD on delegated implementation via the goal prompt

## Would Revise If

- **Event-based**: ≥3 tester reports flagging the Iron Law as obstruction; OR zero RED-GREEN-REFACTOR cycle citations in tester feedback by 2026-11-30 (skill is decorative). First trigger moves the skill to optional installation rather than removing it entirely.
- **Date-based**: 2026-11-30 (v2.0.1 tester-feedback window). If the "When to Use" carve-outs (throwaway prototypes, generated code, configuration files) consistently mis-classify tester work and produce friction, broaden the carve-outs explicitly.
